...
This is "Templates for Reproducible Research Projects in Economics" - a template project demonstrating best practices for reproducible computational research. It uses pytask for workflow orchestration and Pixi for environment management.
...
The project follows a task-based pipeline where each `task_*.py` file defines computational steps:

1. **Data Management** (`src/template_project/data_management/task_data_management_template.py`)
   - Loads raw CSV data, cleans it, saves as pickle to `bld/data/`

2. **Analysis** (`src/template_project/analysis/task_analysis_template.py`)
   - Fits logit models and generates predictions
   - Iterates over `TEMPLATE_GROUPS` defined in `config.py`

3. **Final Outputs** (`src/template_project/final/task_final_template.py`)
...
- Markers: `@pytest.mark.unit`, `@pytest.mark.integration`, `@pytest.mark.end_to_end`, `@pytest.mark.wip`
