...
import pandas as pd
import numpy as np
from {{cookiecutter.project_slug}}.utilities import load_yaml
...
@pytest.fixture
def data():
    data = pd.read_csv("data_fixture.csv")
    return data


@pytest.fixture
def data_info():
    data_info = load_yaml("data_info_fixture.yaml")
    return data_info
...
def test_clean_data_dropna(data, data_info):
    data_clean = clean_data(data, data_info)
    assert not data_clean.isnull().any()
