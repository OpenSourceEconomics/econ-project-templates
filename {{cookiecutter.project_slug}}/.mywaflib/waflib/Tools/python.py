    ...
    if self.install_path:
        if self.install_from:
            pyd = Utils.subst_vars(
                "%s/%s" % (self.install_path, node.path_from(self.install_from)),
                self.env,
            )
        else:
            pyd = Utils.subst_vars(
                "%s/%s" % (self.install_path, node.path_from(self.path)), self.env
            )
    else:
        pyd = node.abspath()
    ...
    for ext in lst:
        if self.env.PYTAG and not self.env.NOPYCACHE:
            ...
            pyobj = (
                ...
                .make_node("%s.%s.%s" % (name, self.env.PYTAG, ext))
            )
            pyobj.parent.mkdir()
        else:
...
def check_python_version(conf, minver=None):
    ...
    if minver is None:
        ...
        conf.msg(
            "Checking for python version >= %s" % (minver_str,),
            pyver_full,
