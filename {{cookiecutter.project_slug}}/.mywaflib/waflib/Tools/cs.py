...
def apply_cs(self):
    ...
    self.cs_task = tsk = self.create_task(
        "mcs", cs_nodes, self.path.find_or_declare(self.gen)
    )
    tsk.env.CSTYPE = "/target:%s" % bintype
    tsk.env.OUT = "/out:%s" % tsk.outputs[0].abspath()
    self.env.append_value(
        "CSFLAGS", "/platform:%s" % getattr(self, "platform", "anycpu")
    )
    ...
    if inst_to:
        ...
        self.install_task = self.add_install_files(
            install_to=inst_to, install_from=self.cs_task.outputs[:], chmod=mod
        )
...
def use_cs(self):
    ...
    for x in names:
        ...
        self.cs_task.set_run_after(
            tsk
        )  # order (redundant, the order is inferred from the nodes inputs/outputs)
