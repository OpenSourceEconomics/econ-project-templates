...
from waflib.Tools import c_tests
...
USELIB_VARS["c"] = set(
    ["INCLUDES", "FRAMEWORKPATH", "DEFINES", "CPPFLAGS", "CCDEPS", "CFLAGS", "ARCH"]
)
USELIB_VARS["cxx"] = set(
    ["INCLUDES", "FRAMEWORKPATH", "DEFINES", "CPPFLAGS", "CXXDEPS", "CXXFLAGS", "ARCH"]
)
USELIB_VARS["d"] = set(["INCLUDES", "DFLAGS"])
USELIB_VARS["includes"] = set(["INCLUDES", "FRAMEWORKPATH", "ARCH"])

USELIB_VARS["cprogram"] = USELIB_VARS["cxxprogram"] = set(
    [
        "LIB",
        "STLIB",
        "LIBPATH",
...
USELIB_VARS["cshlib"] = USELIB_VARS["cxxshlib"] = set(
    [
        "LIB",
        "STLIB",
        "LIBPATH",
...
class link_task(Task.Task):
    ...
    def add_target(self, target):
        ...
        if isinstance(target, str):
            ...
            if self.__class__.__name__.find("shlib") > 0 and getattr(
                self.generator, "vnum", None
            ):
...
def apply_link(self):
    ...
    if inst_to:
        ...
        self.install_task = self.add_install_files(
            install_to=inst_to,
            install_from=self.link_task.outputs[:],
    ...
    if getattr(self, "defs", None) and self.env.DEST_BINFMT == "pe":
        ...
        if self.env.def_PATTERN:
            self.env.append_value(
                "LINKFLAGS", self.env.def_PATTERN % node.path_from(self.get_cwd())
            )
	...
	* ``libfoo.so.14 → libfoo.so.14.15.16``
    ...
    if not re_vnum.match(self.vnum):
        raise Errors.WafError(
            "Invalid vnum %r for target %r" % (self.vnum, getattr(self, "name", self))
        )
    ...
    if getattr(self, "install_task", None):
        ...
        if self.env.DEST_OS == "openbsd":
            ...
            t1 = self.add_install_as(
                install_to=path + os.sep + name3,
                install_from=node,
                chmod=self.link_task.chmod,
            )
            t3 = self.add_symlink_as(
                install_to=path + os.sep + libname, install_from=name3
            )
            if name2 != name3:
                t2 = self.add_symlink_as(
                    install_to=path + os.sep + name2, install_from=name3
                )
