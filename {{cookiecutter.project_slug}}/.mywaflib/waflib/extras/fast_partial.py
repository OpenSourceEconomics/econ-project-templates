        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    Logs.debug(
                        "rev_use: Could not pickle the build cache %s: %r", dbfn, e
                    )
                else:
                    for x in SAVED_ATTRS:
...
def is_stale(self):
    ...
    if (
        ...
        Logs.debug(
            "rev_use: must post %r because the configuration has changed", self.name
        )
        return True
    ...
    try:
        ...
        Logs.debug(
            "rev_use: must post %r because there it has no cached data", self.name
        )
        return True
    ...
    for x in lst:
        ...
        try:
            ...
            Logs.debug(
                "rev_use: must post %r because %r does not exist anymore", self.name, x
            )
            return True
        else:
            if ts != old_ts:
                Logs.debug(
                    "rev_use: must post %r because the timestamp on %r changed %r %r",
                    self.name,
                    x,
                    old_ts,
                    ts,
