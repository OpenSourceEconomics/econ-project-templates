...
def satellite_assembly(self):
    if not getattr(self, "gen", None):
        self.bld.fatal(
            'satellite_assembly needs a template assembly provided with the "gen" parameter'
        )
    ...
    for i, x in enumerate(self.source):
        ...
        if mo:
            ...
            satellite = self.path.find_or_declare(
                os.path.join(templatedir, lang, templatename) + ".resources.dll"
            )
            tsk = self.create_task("al", [resources], [satellite])
            tsk.env.append_value(
                "ALFLAGS", "/template:" + os.path.join(self.path.relpath(), self.gen)
            )
