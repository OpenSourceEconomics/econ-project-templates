...
def find_pyqt5_binaries(self):
    ...
    if getattr(Options.options, "want_pyside2", True):
        ...
        self.find_program(
            ["pylupdate5", "pyside2-lupdate", "pylupdate4"], var="QT_PYLUPDATE"
        )
...
def options(opt):
    ...
    pyqt5opt.add_option(
        ...
        default=False,
        dest="want_pyside2",
        help="use pyside2 bindings as python QT5 bindings (default PyQt5 is searched first, PySide2 after)",
    )
    pyqt5opt.add_option(
        "--pyqt5-pyqt4",
        action="store_true",
        default=False,
        dest="want_pyqt4",
        help="use PyQt4 bindings as python QT5 bindings (default PyQt5 is searched first, PySide2 after, PyQt4 last)",
    )
