...
def parse_rst_node(task, node, nodes, names, seen, dirs=None):
    ...
    code = node.read()
    re_rst = re.compile(
        r"^\s*.. ((?P<subst>\|\S+\|) )?(?P<type>include|image|figure):: (?P<file>.*)$",
        re.M,
    )
...
class docutils(Task.Task):
    ...
    def scan(self):
        ...
        if names:
            Logs.warn(
                "rst: %r: could not find the following file deps: %r", self, names
            )
...
def apply_rst(self):
    ...
    if self.target:
        if isinstance(self.target, Node.Node):
            ...
            self.bld.fatal(
                "rst: Don't know how to build target name %s which is not a string or Node for %s"
                % (self.target, self)
            )
    else:
        tgt = None
    ...
    if tsk_type is not None and tgt is None:
        if tsk_type.startswith("rst2"):
            ...
            self.bld.fatal(
                "rst: Could not detect the output file extension for %s" % self
            )
    ...
    try:
        ...
        self.bld.fatal(
            "rst: Task of type %s not implemented (created by %s)" % (self.type, self)
        )
    ...
    if inst_to:
        self.install_task = self.add_install_files(
            install_to=inst_to, install_from=task.outputs[:]
        )
