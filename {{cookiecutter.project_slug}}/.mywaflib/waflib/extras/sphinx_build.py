        ...
        if path:
            if os.path.isabs(path):
                ...
                if node:
                    ...
                    waflib.Logs.error("Invalid TEXINPUTS folder {}".format(path))
            else:
                waflib.Logs.error(
                    "Cannot resolve relative paths in TEXINPUTS {}".format(path)
                )
    ...
    def scan(self):
        ...
        for doc_name in updated_doc_names:
            for suffix in suffixes:
                ...
                raise waflib.Errors.WafError(
                    "Could not find Sphinx document node at any of: {0}".format(
        ...
        for dependency_paths in app.env.dependencies.values():
            for dependency_path in dependency_paths:
                ...
                if node is None:
                    raise waflib.Errors.WafError(
                        "Could not find Sphinx document dependency node: {0}".format(
                            dependency_path
                        )
                    )
...
def apply_sphinx(task_gen):
    ...
    for requested_builder in requested_builders:
        ...
        if not task_gen.env[tool.upper()]:
            raise waflib.Errors.WafError(
                (
                    "Sphinx '{0}' builder requested " "but '{1}' program not found!"
                ).format(requested_builder, tool)
            )
    ...
    if in_nodes_len != 1:
        raise waflib.Errors.WafError(
            "Sphinx task generator takes one input, {0} given.".format(in_nodes_len)
        )
