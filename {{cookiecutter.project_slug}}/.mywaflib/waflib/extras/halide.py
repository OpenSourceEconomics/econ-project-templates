...
class run_halide_gen(Task.Task):
    ...
    def __str__(self):
        stuff = "halide"
        stuff += "[%s]" % (
            ",".join(("%s=%s" % (k, v)) for k, v in sorted(self.env.env.items()))
        )
        return Task.Task.__str__(self).replace(self.__class__.__name__, stuff)


@TaskGen.feature("halide")
@TaskGen.before_method("process_source")
def halide(self):
    Utils.def_attrs(self, args=[], halide_env={})
...
def options(opt):
    opt.add_option("--halide-root", help="path to Halide include and lib files")
