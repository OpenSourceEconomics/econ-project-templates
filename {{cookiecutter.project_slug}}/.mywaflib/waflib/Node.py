...
class Node(object):
	...
	* The methods bound to a :py:class:`waflib.Build.BuildContext` (require ``bld.srcnode`` and ``bld.bldnode``)
        ...
        if parent:
            if name in parent.children:
                raise Errors.WafError(
                    "node %s exists in the parent files %r already" % (name, parent)
                )
    ...
    def read_json(self, convert=True, encoding="utf-8"):
        ...
        return json.loads(
            self.read(encoding=encoding), object_pairs_hook=object_pairs_hook
        )
    ...
    def write_json(self, data, pretty=True):
        ...
        if not pretty:
            ...
            newline = ""
        output = (
            json.dumps(data, indent=indent, separators=separators, sort_keys=sort_keys)
            + newline
        )
    ...
    def ant_glob(self, *k, **kw):
		...
		* ``..`` find files having two dot characters
        ...
        if kw.get("generator"):
            return Utils.lazy_generator(
                self.ant_iter,
                (ant_sub_matcher, maxdepth, pats, dir, src, remove, quiet),
            )
