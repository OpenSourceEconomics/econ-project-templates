...
def check_waf_version(self, mini="1.9.99", maxi="2.1.0", **kw):
    ...
    if Utils.num2ver(mini) > ver:
        self.fatal(
            "waf version should be at least %r (%r found)" % (Utils.num2ver(mini), ver)
        )
    if Utils.num2ver(maxi) < ver:
        self.fatal(
            "waf version should be at most %r (%r found)" % (Utils.num2ver(maxi), ver)
        )
    self.end_msg("ok", **kw)
...
def find_program(self, filename, **kw):
    ...
    if not kw.get("quiet"):
        self.to_log(
            "find program=%r paths=%r var=%r -> %r" % (filename, path_list, var, ret)
        )
