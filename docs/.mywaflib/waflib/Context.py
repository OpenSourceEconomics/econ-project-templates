    ...
    def __init__(cls, name, bases, dct):
        ...
        try:
            ...
            raise Errors.WafError(
                "Missing command for the context class %r (cmd)" % name
            )
            ...
            if node and (not once or node not in cache):
                ...
                if node and (not once or tup not in cache):
                    ...
                    try:
                        ...
                        user_function = getattr(
                            wscript_module, (name or self.fun), None
                        )
                        if not user_function:
                            if not mandatory:
                                continue
                            raise Errors.WafError(
                                "No function {!r} defined in {}".format(
    ...
    def fatal(self, msg, ex=None):
        ...
        try:
            ...
            if os.environ.get("WAF_PRINT_FAILURE_LOG"):
                # see #1930
                msg = "Log from ({}):\n{}\n".format(logfile, Utils.readf(logfile))
            else:
