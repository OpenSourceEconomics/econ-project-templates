...
def waf_entry_point(current_directory, version, wafdir):
    ...
    if Context.WAFVERSION != version:
        Logs.error(
            "Waf script %r and library %r do not match (directory %r)",
            version,
            Context.WAFVERSION,
            wafdir,
        )
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.basename(
            potential_wscript
        ) == Context.WSCRIPT_FILE and os.path.isfile(potential_wscript):
    ...
    try:
        set_main_module(
            os.path.normpath(os.path.join(Context.run_dir, Context.WSCRIPT_FILE))
        )
...
class Dist(Context.Context):
    ...
    def get_arch_name(self):
        ...
        try:
            ...
            self.arch_name = (
                self.get_base_name() + "." + self.ext_algo.get(self.algo, self.algo)
            )
