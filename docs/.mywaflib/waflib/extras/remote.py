...
class remote(BuildContext):
    ...
    def setup_private_ssh_key(self):
        ...
        for x in self.ssh_hosts, self.ssh_key, self.ssh_config:
            if not os.path.isfile(x):
                ...
                Utils.writef(
                    self.ssh_config, "UserKnownHostsFile %s" % self.ssh_hosts, "wb"
                )
    ...
    def recurse(self, *k, **kw):
        ...
        try:
            ...
            try:
                ...
                self.fatal(
                    "Put a waf file in the directory (./waf-light --tools=remote)"
                )
            else:
                self.env.WAF = "./waf"
    ...
    def make_mkdir_command(self, task):
        return Utils.subst_vars(
            '${SSH} ${SSH_OPTS} ${login} "rm -fr ${remote_dir} && mkdir -p ${remote_dir}"',
            task.env,
        )

    def make_send_command(self, task):
        return Utils.subst_vars(
            '${RSYNC} ${RSYNC_SEND_OPTS} -e "${SSH} ${SSH_OPTS}" ${local_dir} ${login}:${remote_dir}',
            task.env,
        )
    ...
    def make_save_command(self, task):
        return Utils.subst_vars(
            '${RSYNC} ${RSYNC_SAVE_OPTS} -e "${SSH} ${SSH_OPTS}" ${login}:${remote_dir_variant} ${build_dir}',
            task.env,
        )
...
def rsync_and_ssh(task):
    ...
    task.env.remote_dir_variant = (
        f"{task.env.remote_dir}/{Context.g_module.out}/{task.env.variant}"
    )
