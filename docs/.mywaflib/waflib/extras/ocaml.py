...
def mly_hook(self, node):
    mly_task = self.create_task(
        "ocamlyacc", node, [node.change_ext(".ml"), node.change_ext(".mli")]
    )
    ...
    task = self.create_task(
        "ocamlcmi", mly_task.outputs[1], mly_task.outputs[1].change_ext(".cmi")
    )
...
class ocamlx(Task.Task):
    ...
    color = "GREEN"
    run_str = (
        "${OCAMLOPT} ${OCAMLPATH} ${OCAMLFLAGS} ${OCAMLINCLUDES} -c -o ${TGT} ${SRC}"
    )
    scan = scan
...
class ocaml(Task.Task):
    ...
    color = "GREEN"
    run_str = (
        "${OCAMLC} ${OCAMLPATH} ${OCAMLFLAGS} ${OCAMLINCLUDES} -c -o ${TGT} ${SRC}"
    )
    scan = scan
    runnable_status = compile_may_start
