...
RE_EMACS = re.compile(
    r"(?P<filename>.*):(?P<linenum>\d+):  (?P<message>.*)  \[(?P<category>.*)\] \[(?P<confidence>\d+)\]"
)
CPPLINT_RE = {
    "waf": RE_EMACS,
    "emacs": RE_EMACS,
    ...
    "eclipse": re.compile(
        r"(?P<filename>.*):(?P<linenum>\d+): warning: (?P<message>.*)  \[(?P<category>.*)\] \[(?P<confidence>\d+)\]"
    ),
}
...
class cpplint(Task.Task):
    ...
    def run(self):
        global critical_errors
        with cpplint_wrapper(
            get_cpplint_logger(self.env.CPPLINT_OUTPUT),
            self.env.CPPLINT_BREAK,
            self.env.CPPLINT_OUTPUT,
        ):
