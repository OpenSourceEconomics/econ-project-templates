...
class publish(Context.Context):
    ...
    def execute(self):
        ...
        data = safe_urlencode(
            [("pkgdata", fdata), ("pkgname", mod.APPNAME), ("pkgver", mod.VERSION)]
        )
...
class package_reader(Context.Context):
    ...
    def check_errors(self):
        ...
        for c in self.constraints:
            if not c.required_version:
                ...
                if len(reasons) == 1:
                    Logs.error(
                        "%s but no matching package could be found in this repository",
                        reasons[0],
                    )
                else:
    ...
    def load_constraints(self, pkgname, pkgver, requires=REQUIRES):
        try:
            ...
            text = Utils.readf(
                os.path.join(get_distnet_cache(), pkgname, pkgver, requires)
            )
    ...
    def solve(
        ...
        self.trace(f"calling solve with {packages_to_versions!r}    {todo!r} {done!r}")
        done = done + [pkgname]
        ...
        for k in constraints:
            ...
            self.trace(f"domain before {pkgname} -> {k.pkgname}, {domain!r}")

            # apply the constraint
            domain = self.apply_constraint(domain, k)

            self.trace(f"domain after {pkgname} -> {k.pkgname}, {domain!r}")
            ...
            if not domain:
                self.trace(
                    f"no domain while processing constraint {domain!r} from {pkgname!r} {pkgver!r}"
                )
        ...
        return self.solve(
            tmp, n_packages_to_constraints, n_pkgname, n_pkgver, todo[1:], done
        )
    ...
    def local_resolve(self, text):
        self.cache_constraints[(self.myproject, self.myversion)] = parse_constraints(
            text
        )
        p2v = OrderedDict({self.myproject: [self.myversion]})
        (versions, constraints) = self.solve(
            p2v, {}, self.myproject, self.myversion, []
        )
        return self.solution_to_constraints(versions, constraints)

    def download_to_file(self, pkgname, pkgver, subdir, tmp):
        data = safe_urlencode(
            [("pkgname", pkgname), ("pkgver", pkgver), ("pkgfile", subdir)]
        )
