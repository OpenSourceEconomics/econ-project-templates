...
def configure(self):
    ...
    if not isinstance(self.env.PROJ_CONFIGURATION, dict):
        raise Errors.ConfigurationError(
            "The env.PROJ_CONFIGURATION must be a dictionary with at least one key, where each key is the configuration name, and the value is a dictionary of key/value settings."
        )
...
class XCodeNode:
    ...
    def write(self, file):
        if not self._been_written:
            ...
            for attribute, value in self.__dict__.items():
                if attribute[0] != "_":
                    w("\t\t{} = {};\n".format(attribute, self.tostring(value)))
            w("\t};\n\n")
...
class PBXContainerItemProxy(XCodeNode):
    ...
    def __init__(
        self, containerPortal, remoteGlobalIDString, remoteInfo="", proxyType=1
    ):
        XCodeNode.__init__(self)
...
class PBXLegacyTarget(XCodeNode):
    def __init__(self, action, target=""):
        ...
        if not target:
            self.buildArgumentsString = "{} {}".format(sys.argv[0], action)
        else:
            self.buildArgumentsString = "{} {} --targets={}".format(
                sys.argv[0], action, target
            )
        self.buildPhases = []
...
class PBXShellScriptBuildPhase(XCodeNode):
    def __init__(self, action, target):
        ...
        self.shellScript = "{} {} {} --targets={}".format(
            sys.executable, sys.argv[0], action, target
        )
...
class PBXNativeTarget(XCodeNode):
    ...
    def __init__(
        ...
        self.productReference = PBXFileReference(
            node.name, node.abspath(), file_type, ""
        )
    ...
    def add_build_phase(self, phase):
        ...
        if (phase.__class__ == PBXFrameworksBuildPhase) or (
            phase.__class__ == PBXSourcesBuildPhase
        ):
    ...
    def __init__(self, name, version, env):
        ...
        if not isinstance(env.PROJ_CONFIGURATION, dict):
            raise Errors.WafError(
                "Error: env.PROJ_CONFIGURATION must be a dictionary. This is done for you if you do not define one yourself. However, did you load the xcode module at the end of your wscript configure() ?"
            )
...
def process_xcode(self):
    ...
    if target_type not in TARGET_TYPES:
        raise Errors.WafError(
            "Target type '{}' does not exists. Available options are '{}'. In target '{}'".format(
    ...
    files = [
        p.mainGroup.find_fileref(PBXFileReference(n.name, n.abspath())) for n in hdrs
    ]
    ...
    bldsettings = {
        ...
        + Utils.to_list(self.env.LIBDIR),
        "FRAMEWORK_SEARCH_PATHS": ["$(inherited)"]
        + Utils.to_list(self.env.FRAMEWORKPATH),
