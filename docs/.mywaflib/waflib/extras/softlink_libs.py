...
def options(opt):
    ...
    grp.add_option(
        "--exclude",
        default="/usr/lib,/lib",
        help="No symbolic links are created for libs within [%default]",
    )
        ...
        for g in bld.groups:
            for tgen in g:
                ...
                linked += [
                    ...
                    in ["cprogram", "cshlib", "cxxprogram", "cxxshlib"]
                ]
        lib_list = []
        if len(linked):
    ...
    def run(self):
        ...
        for lib in lib_list:
            self.generator.bld.symlink_as(
                "${LIBDIR}/" + basename(lib), lib, postpone=False
            )
        return 0
