...
def options(opt):
    ...
    grp.add_option(
        "--oracle_home",
        action="store",
        default=environ.get("PROC_ORACLE"),
        help="Path to Oracle installation home (has bin/lib)",
    )
    grp.add_option(
        "--tns_admin",
        action="store",
        default=environ.get("TNS_ADMIN"),
        help="Directory containing server list (TNS_NAMES.ORA)",
    )
    grp.add_option(
        "--connection",
        action="store",
        default="dummy-user/dummy-password@dummy-server",
        help="Format: user/password@server",
    )
