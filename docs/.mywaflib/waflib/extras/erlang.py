...
def add_erl_test_run(self):
    ...
    test_list = ", ".join(
        [m.change_ext("").path_from(test_task.cwd) + ":test()" for m in test_modules]
    )
    test_flag = (
        "halt(case lists:all(fun(Elem) -> Elem == ok end, [%s]) of true -> 0; false -> 1 end)."
        % test_list
    )
...
def add_edoc_task(self):
    ...
    tsk.env.append_value(
        "ERL_DOC_FLAGS", 'edoc:files(["%s"]), halt(0).' % tsk.inputs[0].abspath()
    )
