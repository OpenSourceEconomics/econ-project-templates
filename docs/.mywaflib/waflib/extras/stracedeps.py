...
re_lines = re.compile(
    fr"^(?P<pid>\d+)\s+(?:(?:{s_file})|(?:{s_process}))\r*$",
...
def task_method(func):
    ...
    try:
        setattr(
            Task.Task, "nostrace_%s" % func.__name__, getattr(Task.Task, func.__name__)
        )
...
def get_strace_args(self):
    return (self.env.STRACE or ["strace"]) + [
        "-e",
        TRACECALLS,
        "-f",
        "-o",
        self.get_strace_file(),
    ]
