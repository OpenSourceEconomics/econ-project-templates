...
def process_protoc(self, node):
    ...
    if "javac" in self.features:
        ...
        if javacn:
            ...
            if (
                self.env.PROTOC_MAJOR > "2"
                and node.abspath()[
                    node.abspath().rfind(os.sep) + 1 : node.abspath().rfind(".")
                ].title()
                in messages
            ):
                nodename += (
    ...
    if not out_nodes:
        raise Errors.WafError(
            "Feature %r not supported by protoc extra" % self.features
        )
...
def configure(conf):
    conf.check_cfg(
        package="protobuf", uselib_store="PROTOBUF", args=["--cflags", "--libs"]
    )
