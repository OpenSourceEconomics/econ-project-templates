...
class qxx(Task.classes["cxx"]):
    ...
    def create_moc_task(self, h_node, m_node):
        ...
        try:
            ...
            tsk = moc_cache[h_node] = Task.classes["moc"](
                env=self.env, generator=self.generator
            )
...
def create_uic_task(self, node):
    ...
    uictask.outputs = [
        self.path.find_or_declare(self.env["ui_PATTERN"] % node.name[:-3])
    ]
    ...
    if getattr(self, "lang", None):
        ...
        if getattr(self, "update", None) and Options.options.trans_qt4:
            cxxnodes = [a.inputs[0] for a in self.compiled_tasks] + [
                a.inputs[0]
                for a in self.tasks
                if getattr(a, "inputs", None) and a.inputs[0].name.endswith(".ui")
            ]
            for x in qmtasks:
...
class rcc(Task.Task):
    ...
    def scan(self):
        ...
        if not has_xml:
            Logs.error(
                "no xml support was found, the rcc dependencies will be incomplete!"
            )
            return ([], [])
...
class qm2rcc(Task.Task):
    ...
    def run(self):
        ...
        txt = "\n".join(
            [
                "<file>%s</file>" % k.path_from(self.outputs[0].parent)
...
def find_qt4_binaries(self):
    ...
    qtbin = (
        self.cmd_and_log(self.env.QMAKE + ["-query", "QT_INSTALL_BINS"]).strip()
        + os.sep
    )
    ...
    uicver = uicver.replace("Qt User Interface Compiler ", "").replace(
        "User Interface Compiler for Qt", ""
    )
    self.end_msg(uicver)
    if uicver.find(" 3.") != -1:
...
def find_qt4_libraries(self):
    ...
    if not qtlibs:
        try:
            qtlibs = self.cmd_and_log(
                self.env.QMAKE + ["-query", "QT_INSTALL_LIBS"]
            ).strip()
        except Errors.WafError:
            qtdir = (
                self.cmd_and_log(
                    self.env.QMAKE + ["-query", "QT_INSTALL_PREFIX"]
                ).strip()
                + os.sep
            )
    ...
    qtincludes = (
        os.environ.get("QT4_INCLUDES")
        or self.cmd_and_log(self.env.QMAKE + ["-query", "QT_INSTALL_HEADERS"]).strip()
    )
    env = self.env
    if not "PKG_CONFIG_PATH" in os.environ:
        os.environ[
            "PKG_CONFIG_PATH"
        ] = f"{qtlibs}:{qtlibs}/pkgconfig:/usr/lib/qt4/lib/pkgconfig:/opt/qt4/lib/pkgconfig:/usr/lib/qt4/lib:/opt/qt4/lib"
    ...
    try:
        ...
        for i in self.qt4_vars:
            ...
            if Utils.unversioned_sys_platform() == "darwin":
                ...
                env.append_unique(
                    "INCLUDES_" + uselib, os.path.join(qtlibs, frameworkName, "Headers")
                )
                ...
                for k in ("lib%s.a", "lib%s4.a", "%s.lib", "%s4.lib"):
                    ...
                    if os.path.exists(lib):
                        env.append_unique(
                            "LIB_" + uselib, i + k[k.find("%s") + 2 : k.find(".")]
                        )
