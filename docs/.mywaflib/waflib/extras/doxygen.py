...
class doxygen(Task.Task):
        ...
        if not getattr(self, "pars", None):
            ...
            if self.pars.get("OUTPUT_DIRECTORY"):
                ...
                output_node = (
                    self.inputs[0]
                    .parent.get_bld()
                    .make_node(self.pars["OUTPUT_DIRECTORY"])
                )
            else:
                ...
                output_node = (
                    self.inputs[0]
                    .parent.get_bld()
                    .make_node(self.inputs[0].name + ".doxy")
                )
            output_node.mkdir()
                ...
                for i in self.pars.get("INPUT").split():
                    ...
                    if not node:
                        self.generator.bld.fatal(
                            "Could not find the doxygen input %r" % i
                        )
    ...
    def scan(self):
        ...
        exclude_patterns = [
            pattern.replace("*/", "**/") for pattern in exclude_patterns
        ]
    ...
    def run(self):
        dct = self.pars.copy()
        code = "\n".join(["{} = {}".format(x, dct[x]) for x in self.pars])
    ...
    def add_install(self):
        ...
        if getattr(self.generator, "install_path", None):
            if not getattr(self.generator, "doxy_tar", None):
                self.generator.add_install_files(
                    install_to=self.generator.install_path,
