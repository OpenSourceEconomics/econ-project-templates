...
def options(opt):
    ...
    opt.add_option(
        "--cppcheck-lib-enable",
        dest="cppcheck_lib_enable",
        default="warning,performance,portability,style",
        action="store",
    ...
    opt.add_option(
        "--cppcheck-max-configs",
        dest="cppcheck_max_configs",
        default="20",
        action="store",
...
def configure(conf):
    ...
    if conf.options.cppcheck_jobs != "1" and (
        "unusedFunction" in conf.options.cppcheck_bin_enable
        or "unusedFunction" in conf.options.cppcheck_lib_enable
        or "all" in conf.options.cppcheck_bin_enable
        or "all" in conf.options.cppcheck_lib_enable
    ):
        Logs.warn(
            "cppcheck: unusedFunction cannot be used with multiple threads, cppcheck will disable it automatically"
        )
...
class cppcheck(Task.Task):
    ...
    def run(self):
        stderr = self.generator.bld.cmd_and_log(
            self.cmd, quiet=Context.STDERR, output=Context.STDERR
        )
    ...
    def _create_html_file(self, sourcefile, htmlfile, errors):
        ...
        for div in page.findall("div"):
            ...
            if div.get("id") == "content":
                ...
                formatter = CppcheckHtmlFormatter(
                    linenos=True,
                    style="colorful",
                    hl_lines=hl_lines,
                    lineanchors="line",
    ...
    def _create_html_table(self, content, files):
        ...
        for name, val in files.items():
            ...
            errors = sorted(
                val["errors"],
                key=lambda e: int(e["line"]) if "line" in e else sys.maxint,
            )
            for e in errors:
                if not "line" in e:
                    ...
                    if e["severity"] == "error":
                        attr = 'class="error"'
                    s = '<tr><td><a href="{}#line-{}">{}</a></td>'.format(
                        f, e["line"], e["line"]
                    )
    ...
    def _errors_evaluate(self, errors, http_index):
        ...
        if set(fatal) & set(severity):
            exc = "\n"
            exc += (
                "\nccpcheck detected fatal error(s) in task '%s', see report for details:"
                % name
            )
            exc += "\n    file://%r" % (http_index)
            exc += "\n"
            ...
            msg = (
                "\nccpcheck detected (possible) problem(s) in task '%s', see report for details:"
                % name
            )
            msg += "\n    file://%r" % http_index
            msg += "\n"
...
class CppcheckHtmlFormatter(pygments.formatters.HtmlFormatter):
    ...
    def wrap(self, source, outfile):
        line_no = 1
        for i, t in super().wrap(source, outfile):
...
CCPCHECK_HTML_TYPE = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">\n'
