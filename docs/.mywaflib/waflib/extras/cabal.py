...
def configure(self):
    ...
    if self.root.find_node(pkgconfd + "/package.cache"):
        ...
        self.msg(
            "Created package database", pkgconfd, color="YELLOW" if pkgdir else "GREEN"
        )
...
class cabal_configure(Cabal):
    ...
    def scan(self):
        ...
        deps = [
            out_node.find_or_declare(dep).change_ext(".package")
            for dep in Utils.to_list(self.depends_on)
        ]
        return (deps, ())
