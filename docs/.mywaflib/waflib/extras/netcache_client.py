...
def check_cache(conn, ssig):
    ...
    if time.time() - all_sigs_in_cache[0] > STALE_TIME:
        ...
        while cnt < size:
            ...
            if not data:
                raise ValueError(f"connection ended {cnt!r} {size!r}")
            buf.append(data)
            cnt += len(data)
...
def build(bld):
    if (
        not "NETCACHE" in os.environ
        and not "NETCACHE_PULL" in os.environ
        and not "NETCACHE_PUSH" in os.environ
    ):
        Logs.warn(
            "Setting  NETCACHE_PULL=127.0.0.1:11001 and NETCACHE_PUSH=127.0.0.1:12001"
        )
