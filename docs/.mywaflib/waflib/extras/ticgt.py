...
def find_ticc(conf):
    conf.find_program(
        ["cl6x"],
        var="CC",
        path_list=opj(getattr(Options.options, "ti-cgt-dir", ""), "bin"),
    )
...
def options(opt):
    opt.add_option(
        ...
        type="string",
        dest="ti-cgt-dir",
        help="Specify alternate cgt root folder",
        default="",
    )
    opt.add_option(
        "--with-ti-biosutils",
        type="string",
        dest="ti-biosutils-dir",
        help="Specify alternate biosutils folder",
        default="",
    )
    opt.add_option(
        "--with-ti-dspbios",
        type="string",
        dest="ti-dspbios-dir",
        help="Specify alternate dspbios folder",
        default="",
    )
    opt.add_option(
        "--with-ti-dsplink",
        type="string",
        dest="ti-dsplink-dir",
        help="Specify alternate dsplink folder",
        default="",
    )
    opt.add_option(
        "--with-ti-xdctools",
        type="string",
        dest="ti-xdctools-dir",
        help="Specify alternate xdctools folder",
        default="",
    )
...
def apply_tconf(self):
    sources = [
        x.get_src() for x in self.to_nodes(self.source, path=self.path.get_src())
    ]
    node = sources[0]
