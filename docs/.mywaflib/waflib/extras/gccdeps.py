...
re_splitter = re.compile(
    r"(?<!\\)\s+"
)  # split by space, except when spaces are escaped
...
def post_run(self):
    ...
    for x in val:
        ...
        if not node:
            raise ValueError(f"could not find {x!r} for {self!r}")
...
def configure(conf):
    ...
    if conf.env.CC_NAME in supported_compilers:
        try:
            conf.check(
                ...
                features="c force_gccdeps",
                cflags=flags,
                msg="Checking for c flags %r" % "".join(flags),
            )
    ...
    if conf.env.CXX_NAME in supported_compilers:
        try:
            conf.check(
                fragment="int main() { return 0; }",
                features="cxx force_gccdeps",
                cxxflags=flags,
                msg="Checking for cxx flags %r" % "".join(flags),
            )
        except Errors.ConfigurationError:
            pass
        else:
