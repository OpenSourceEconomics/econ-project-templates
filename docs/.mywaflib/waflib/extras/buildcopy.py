    ...
    def to_src_nodes(lst):
        ...
        if isinstance(lst, Node.Node):
            ...
            if not os.path.isfile(lst.abspath()):
                raise Errors.WafError(
                    "buildcopy: Cannot copy directory %s (unsupported action)" % lst
                )
            return lst
        ...
        if node:
            if not os.path.isfile(node.abspath()):
                raise Errors.WafError(
                    "buildcopy: Cannot copy directory %s (unsupported action)" % node
                )
            return node
        ...
        if node:
            ...
            return node
        raise Errors.WafError(
            "buildcopy: File not found in src: %s" % os.path.join(*lst)
        )

    nodes = [
        to_src_nodes(n)
        for n in getattr(self, "buildcopy_source", getattr(self, "source", []))
    ]
    node_pairs = [(n, n.get_bld()) for n in nodes]
    self.create_task(
        "buildcopy",
        [n[0] for n in node_pairs],
        [n[1] for n in node_pairs],
