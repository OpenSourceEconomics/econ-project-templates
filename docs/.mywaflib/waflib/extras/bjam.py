...
def options(opt):
    ...
    grp.add_option(
        "--bjam_src", default=None, help="You can find it in <boost root>/tools/jam/src"
    )
    grp.add_option(
        "--bjam_uname",
        default="linuxx86_64",
        help="bjam is built in <src>/bin.<uname>/bjam",
    )
...
def configure(cnf):
    ...
    try:
        cnf.find_program(
            "bjam", path_list=[cnf.env.BJAM_SRC + sep + "bin." + cnf.env.BJAM_UNAME]
        )
    except Exception:
        cnf.env.BJAM = None
...
class bjam_build(Task):
    ...
    def run(self):
        ...
        if ret != 0:
            return ret
        self.set_outputs(
            path.get_bld().ant_glob("lib/*") + path.get_bld().ant_glob("bin/*")
        )
        return 0
