...
def boost_get_version(self, d):
    ...
    return self.check_cxx(
        fragment=BOOST_VERSION_CODE, includes=[d], execute=True, define_ret=True
    ).split(":")
...
def __boost_get_libs_path(self, *k, **kw):
    ...
    if not path:
        if libs:
            ...
            self.end_msg(
                "libs not found, please provide a --boost-libs argument (see help)"
            )
            self.fatal("The configuration failed")
...
def boost_get_libs(self, *k, **kw):
    ...
    def match_libs(lib_names, is_static):
        ...
        if kw.get("mt", False):
            t.append("-mt")
        if kw.get("abi"):
            t.append("{}{}".format(is_static and "-s" or "-", kw["abi"]))
        elif is_static:
            t.append("-s")
        ...
        for lib in lib_names:
            if lib == "python":
                ...
                tags = "({0})?((-py{2})|(-py{1}(?=[^0-9]))|({2})|(-{1}.{3})|({1}(?=[^0-9]))|(?=[^0-9])(?!-py))".format(
                    tags_pat, kw["python"][0], kw["python"], kw["python"][1]
                )
            else:
                tags = tags_pat
            # Trying libraries, from most strict match to least one
            for pattern in [
                f"boost_{lib}{toolset_pat}{tags}{version}{ext}$",
                f"boost_{lib}{tags}{version}{ext}$",
                # Give up trying to find the right version
                f"boost_{lib}{toolset_pat}{tags}{ext}$",
                f"boost_{lib}{tags}{ext}$",
                f"boost_{lib}{ext}$",
                "boost_%s" % lib,
            ]:
                ...
                self.end_msg(f"lib {lib} not found in {path.abspath()}")
                self.fatal("The configuration failed")
        return libs

    return (
        path.abspath(),
        match_libs(kw.get("lib"), False),
        match_libs(kw.get("stlib"), True),
    )
...
def check_boost(self, *k, **kw):
    ...
    params = {"lib": k and k[0] or kw.get("lib"), "stlib": kw.get("stlib")}
        ...
        if toolset in ("vc",):
            ...
            for cxxflags in (["/MD", "/EHsc"], []):
                ...
                try:
                    ...
                    self.end_msg(
                        "ok: winning cxxflags combination: %s"
...
def install_boost(self):
    if (
        install_boost.done
        or not Utils.is_win32
        or not self.bld.cmd.startswith("install")
    ):
        return
    ...
    for lib in self.env.LIB_BOOST:
        try:
            file = self.bld.find_file(
                self.env.cxxshlib_PATTERN % lib, self.env.LIBPATH_BOOST
            )
            self.add_install_files(
                install_to=inst_to, install_from=self.bld.root.find_node(file)
            )
        except:
            continue
