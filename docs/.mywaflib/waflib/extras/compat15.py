...
def cmd_output(cmd, **kw):
    ...
    if p.returncode:
        if not silent:
            msg = "command execution failed: {} -> {!r}".format(cmd, str(output))
...
Configure.ConfigurationContext.check_message_1 = (
    Configure.ConfigurationContext.start_msg
)
...
def post(self):
    ...
    if "cstaticlib" in self.features:
        if Logs.verbose:
            Logs.warn(
                'compat: the feature cstaticlib does not exist anymore (use "cstlib" or "cxxstlib")'
            )
