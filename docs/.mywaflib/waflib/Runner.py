                ...
                try:
                    ...
                    if cond:
                        ...
                        for tsk in self.postponed:
                            ...
                            lst.append("{}\t-> {!r}".format(repr(tsk), deps))
                            if not deps:
                                lst.append(
                                    "\n  task %r dependencies are done, check its *runnable_status*?"
                                    % id(tsk)
                                )
                        raise Errors.WafError(
                            "Deadlock detected: check the task build order%s"
                            % "".join(lst)
                        )
            ...
            if self.postponed:
                ...
                if self.incomplete:
                    for x in self.incomplete:
                        ...
                        raise Errors.WafError(
                            "Broken revdeps detected on %r" % self.incomplete
                        )
                else:
            ...
            if tmp[n] == 0:
                ...
                for tsk in acc:
                    ...
                    if tsk is n:
                        ...
                        break
                raise Errors.WafError(
                    'Task dependency cycle in "run_after" constraints: %s'
                    % "".join(lst)
                )
