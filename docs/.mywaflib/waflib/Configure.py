...
class ConfigurationContext(Context.Context):
    ...
    def init_dirs(self):
        ...
        if not out:
            out = Options.lockfile.replace(".lock-waf_%s_" % sys.platform, "").replace(
                ".lock-waf", ""
            )
        ...
        if not os.path.isdir(self.bldnode.abspath()):
            conf.fatal(
                "Could not create the build directory %s" % self.bldnode.abspath()
            )
    ...
    def store(self):
        ...
        n.write(f"version = 0x{Context.HEXVERSION:x}\ntools = {self.tools!r}\n")
        ...
        for key in self.all_envs:
            ...
            tmpenv.store(
                os.path.join(self.cachedir.abspath(), key + Build.CACHE_SUFFIX)
            )
    ...
    def load(self, tool_list, tooldir=None, funs=None, with_sys_path=True, cache=False):
        ...
        for tool in tools:
            ...
            module = None
            try:
                module = Context.load_tool(
                    tool, tooldir, ctx=self, with_sys_path=with_sys_path
                )
                ...
                self.to_log(f"imp {tool!r} ({tooldir!r} & {funs!r})")
...
def check_waf_version(self, mini="1.9.99", maxi="2.1.0", **kw):
    ...
    if Utils.num2ver(mini) > ver:
        self.fatal(
            "waf version should be at least {!r} ({!r} found)".format(
                Utils.num2ver(mini), ver
            )
        )
    if Utils.num2ver(maxi) < ver:
        self.fatal(
            "waf version should be at most {!r} ({!r} found)".format(
                Utils.num2ver(maxi), ver
            )
        )
    self.end_msg("ok", **kw)
