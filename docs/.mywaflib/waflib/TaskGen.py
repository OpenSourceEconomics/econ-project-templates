...
class task_gen:
	...
	* The attribute 'idx' is a counter of task generators in the same path
    ...
    def __str__(self):
        """Debugging helper"""
        return f"<task_gen {self.name!r} declared in {self.path.abspath()}>"
    ...
    def __repr__(self):
        ...
        for x in self.__dict__:
            if x not in ("env", "bld", "compiled_tasks", "tasks"):
                lst.append("{}={}".format(x, repr(getattr(self, x))))
        return "bld({}) in {}".format(", ".join(lst), self.path.abspath())
    ...
    def post(self):
        ...
        for x in self.features:
            ...
            if st:
                ...
                Logs.warn(
                    "feature %r does not exist - bind at least one method to it?", x
                )
        ...
        if prec:
            ...
            for k, v in prec.items():
                buf.append("- {} after {}".format(k, [x for x in v if x in prec]))
    ...
    def get_hook(self, node):
        ...
        raise Errors.WafError(
            f"File {node!r} has no mapping in {keys!r} (load a waf tool?)"
        )
    ...
    def x_file(self, node):
        ...
        if install_path:
            self.install_task = self.add_install_files(
                install_to=install_path, install_from=tsk.outputs
            )
        return tsk
...
def to_nodes(self, lst, path=None):
    ...
    for x in Utils.to_list(lst):
        if isinstance(x, str):
            ...
            continue
        if not node:
            raise Errors.WafError(f"source not found: {x!r} in {self!r}")
        tmp.append(node)
    return tmp
...
class subst_pc(Task.Task):
    ...
    def run(self):
        ...
        code = self.inputs[0].read(
            encoding=getattr(self.generator, "encoding", "latin-1")
        )
        if getattr(self.generator, "subst_fun", None):
            code = self.generator.subst_fun(self, code)
            if code is not None:
                self.outputs[0].write(
                    code, encoding=getattr(self.generator, "encoding", "latin-1")
                )
            self.force_permissions()
            return None
        ...
        try:
            ...
            for x in lst:
                tmp = (
                    getattr(self.generator, x, "") or self.env[x] or self.env[x.upper()]
                )
                try:
                    tmp = "".join(tmp)
                except TypeError:
