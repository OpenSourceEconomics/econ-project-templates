...
class BuildContext(Context.Context):
    ...
    def __init__(self, **kw):
        ...
        r"""List of targets to build (default: \*)"""
        ...
        for x in lst:
            ...
            for f in env[CFG_FILES]:
                ...
                if not newnode or not newnode.exists():
                    raise Errors.WafError(
                        "Missing configuration file %r, reconfigure the project!" % f
                    )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    Logs.debug(
                        "build: Could not pickle the build cache %s: %r", dbfn, e
                    )
                else:
    ...
    def get_tgen_by_name(self, name):
        ...
        try:
            ...
            raise Errors.WafError(
                "Could not find a task generator for the name %r" % name
            )
...
class inst(Task.Task):
    ...
    def get_install_path(self, destdir=True):
        ...
        if destdir and Options.options.destdir:
            dest = os.path.join(
                Options.options.destdir, os.path.splitdrive(dest)[1].lstrip(os.sep)
            )
        return dest
    ...
    def run_now(self):
        ...
        if status not in (Task.RUN_ME, Task.SKIP_ME):
            raise Errors.TaskNotReady(f"Could not process {self!r}: status {status!r}")
        self.run()
    ...
    def fix_perms(self, tgt):
        ...
        if not Utils.is_win32:
            user = getattr(self, "install_user", None) or getattr(
                self.generator, "install_user", None
            )
            group = getattr(self, "install_group", None) or getattr(
                self.generator, "install_group", None
            )
            if user or group:
    ...
    def do_uninstall(self, src, tgt, lbl, **kw):
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                ...
                if Logs.verbose > 1:
                    Logs.warn(
                        "Could not remove %s (error code %r)", e.filename, e.errno
                    )
    ...
    def clean(self):
        ...
        if hasattr(self, "clean_files"):
            ...
            for n in self.bldnode.ant_glob(
                "**/*", excl=".lock* *conf_check_*/** config.log c4che/*", quiet=True
            ):
                if n in lst:
                    continue
                n.delete()
