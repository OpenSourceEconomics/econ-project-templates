...
def listdir_win32(s):
    ...
    if not s:
        try:
            ...
            ndrives = ctypes.windll.kernel32.GetLogicalDriveStringsA(
                maxdrives * dlen, ctypes.byref(buf)
            )
            return [
                str(buf.raw[4 * i : 4 * i + 2].decode("ascii"))
                for i in range(int(ndrives / dlen))
            ]
...
def split_path_msys(path):
    if path.startswith(("/", "\\")) and not path.startswith(("//", "\\\\")):
        ...
        if not msysroot:
            ...
            msysroot = subprocess.check_output(["cygpath", "-w", "/"]).decode(
                sys.stdout.encoding or "latin-1"
            )
...
def get_process():
    ...
    try:
        ...
        return subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stdin=subprocess.PIPE, bufsize=0
        )
