...
def gather_ifort_versions(conf, versions):
    ...
    try:
        ...
        try:
            all_versions = Utils.winreg.OpenKey(
                Utils.winreg.HKEY_LOCAL_MACHINE, "SOFTWARE\\Intel\\Compilers\\Fortran"
            )
        except OSError:
            return
    index = 0
...
def setup_ifort(conf, versiondict):
    ...
    desired_versions = conf.env.MSVC_VERSIONS or list(
        reversed(list(versiondict.keys()))
    )
    ...
    conf.fatal(
        "ifort: Impossible to find a valid architecture for building {!r} - {!r}".format(
...
""".format(
    ...
    if None in (MSVC_PATH, MSVC_INCDIR, MSVC_LIBDIR):
        conf.fatal(
            "ifort: Could not find a valid architecture for building (get_ifort_version_win32)"
        )
    ...
    try:
        ...
        Logs.debug(
            "ifort: get_ifort_version: %r %r %r -> failure %s",
            compiler,
            version,
            target,
            str(e),
        ...
        conf.fatal(
            "ifort: cannot run the compiler in get_ifort_version (run with -v to display errors)"
        )
    else:
        Logs.debug(
            "ifort: get_ifort_version: %r %r %r -> OK", compiler, version, target
        )
    finally:
...
class target_compiler:
    ...
    def evaluate(self):
        ...
        self.is_done = True
        try:
            vs = self.conf.get_ifort_version_win32(
...
def apply_manifest_ifort(self):
    ...
    if (
        self.env.IFORT_WIN32
        and self.env.IFORT_MANIFEST
