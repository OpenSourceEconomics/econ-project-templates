...
def options(opt):
    opt.add_option(
        "--msvc_version",
        type="string",
        help='msvc version, eg: "msvc 10.0,msvc 9.0"',
        default="",
    )
    opt.add_option(
        "--msvc_targets", type="string", help='msvc targets, eg: "x64,arm"', default=""
    )
    opt.add_option(
        "--no-msvc-lazy",
        action="store_false",
        help="lazily check msvc target environments",
        default=True,
        dest="msvc_lazy",
    )
...
def setup_msvc(conf, versiondict):
    ...
    if platforms == [""]:
        platforms = Utils.to_list(conf.env.MSVC_TARGETS) or [
            i for i, j in all_msvc_platforms + all_icl_platforms + all_wince_platforms
        ]
    desired_versions = getattr(Options.options, "msvc_version", "").split(",")
    if desired_versions == [""]:
        desired_versions = conf.env.MSVC_VERSIONS or list(
            reversed(sorted(versiondict.keys()))
        )
    ...
    conf.fatal(
        "msvc: Impossible to find a valid architecture for building {!r} - {!r}".format(
...
""".format(
    ...
    try:
        ...
        Logs.debug(
            "msvc: get_msvc_version: %r %r %r -> failure %s",
            compiler,
            version,
            target,
            str(e),
    ...
    for vcver, vcvar in (("VCExpress", "Exp"), ("VisualStudio", "")):
        ...
        try:
            ...
            try:
                all_versions = Utils.winreg.OpenKey(
                    Utils.winreg.HKEY_LOCAL_MACHINE, prefix
                )
            except OSError:
                continue
        ...
        while 1:
            ...
            detected_versions.append(
                (versionnumber, version + vcvar, prefix + "\\" + version)
            )
...
class target_compiler:
    ...
    def evaluate(self):
        ...
        self.is_done = True
        try:
            vs = self.conf.get_msvc_version(
                self.compiler, self.version, self.bat_target, self.bat
            )
    ...
    while 1:
        ...
        if path and os.path.isfile(os.path.join(path, "bin", "SetEnv.cmd")):
            ...
            for target, arch in all_msvc_platforms:
                targets[target] = target_compiler(
                    conf,
                    "wsdk",
                    arch,
                    version,
...
def gather_msvc_targets(conf, versions, version, vc_path):
    ...
    if os.path.isfile(
        os.path.join(vc_path, "VC", "Auxiliary", "Build", "vcvarsall.bat")
    ):
        ...
        targets["x86"] = target_compiler(
            ...
            version,
            "",
            os.path.join(vc_path, "Bin", "vcvars32.bat"),
        )
    if targets:
    ...
    for device, platforms in supported_platforms:
        ...
        for platform, compiler, include, lib in platforms:
            ...
            if os.path.isdir(os.path.join(winCEpath, "lib", platform)):
                ...
                targets[platform] = target_compiler(
                    conf, "msvc", platform, version, "x86", vsvars, combine_common
                )
        if targets:
...
def gather_winphone_targets(conf, versions, version, vc_path, vsvars):
    ...
    for target, realtarget in all_msvc_platforms[::-1]:
        targets[target] = target_compiler(
            conf, "winphone", realtarget, version, target, vsvars
        )
    if targets:
...
def gather_icl_versions(conf, versions):
    ...
    try:
        ...
        try:
            all_versions = Utils.winreg.OpenKey(
                Utils.winreg.HKEY_LOCAL_MACHINE, "SOFTWARE\\Intel\\Compilers\\C++"
            )
        except OSError:
            return
    index = 0
        ...
        for target, arch in all_icl_platforms:
            ...
            try:
                ...
                if os.path.isfile(batch_file):
                    targets[target] = target_compiler(
                        conf, "intel", arch, version, target, batch_file
                    )
        for target, arch in all_icl_platforms:
            try:
                icl_version = Utils.winreg.OpenKey(
                    all_versions, version + "\\" + target
                )
                path, type = Utils.winreg.QueryValueEx(icl_version, "ProductDir")
            except OSError:
                continue
...
def gather_intel_composer_versions(conf, versions):
    ...
    try:
        all_versions = Utils.winreg.OpenKey(
            Utils.winreg.HKEY_LOCAL_MACHINE, "SOFTWARE\\Wow6432node\\Intel\\Suites"
        )
    except OSError:
        try:
            all_versions = Utils.winreg.OpenKey(
            ...
            try:
                try:
                    defaults = Utils.winreg.OpenKey(
                        all_versions, version + "\\Defaults\\C++\\" + targetDir
                    )
                except OSError:
                ...
                if version[0:2] == "13" and getattr(
                    ...
                    compilervars_arch = os.path.join(
                        path, "bin", "compilervars_arch.bat"
                    )
                    for vscomntool in ("VS110COMNTOOLS", "VS100COMNTOOLS"):
                        if vscomntool in os.environ:
                            vs_express_path = (
                                os.environ[vscomntool] + r"..\IDE\VSWinExpress.exe"
                            )
                            dev_env_path = os.environ[vscomntool] + r"..\IDE\devenv.exe"
                            if (
                                r'if exist "%VS110COMNTOOLS%..\IDE\VSWinExpress.exe"'
...
def visual_studio_add_flags(self):
    ...
    if self.environ.get("INCLUDE"):
        v.prepend_value(
            "INCLUDES", [x for x in self.environ["INCLUDE"].split(";") if x]
        )  # notice the 'S'
...
def msvc_common_flags(conf):
    ...
    if (v.MSVC_COMPILER == "msvc" and v.MSVC_VERSION >= 8) or (
        v.MSVC_COMPILER == "wsdk" and v.MSVC_VERSION >= 6
    ):
