...
def check_fortran_dummy_main(self, *k, **kw):
    ...
    for main in lst:
        ...
        self.fatal(
            "could not detect whether fortran requires a dummy main, see the config.log"
        )
...
def check_fortran_verbose_flag(self, *k, **kw):
    ...
    for x in ("-v", "--verbose", "-verbose", "-V"):
        try:
            ...
            if self.is_link_verbose(self.test_bld.err) or self.is_link_verbose(
                self.test_bld.out
            ):
                self.end_msg(x)
                break
...
def check_fortran_clib(self, autoadd=True, *k, **kw):
    ...
    if not self.env.FC_VERBOSE_FLAG:
        self.fatal(
            "env.FC_VERBOSE_FLAG is not set: execute check_fortran_verbose_flag?"
        )
...
def check_fortran_mangling(self, *k, **kw):
    ...
    if not self.env.FC_MAIN:
        self.fatal(
            'Checking for mangling requires self.env.FC_MAIN (execute "check_fortran_dummy_main" first?)'
        )
