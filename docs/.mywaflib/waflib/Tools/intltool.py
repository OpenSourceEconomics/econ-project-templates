...
def apply_intltool_in_f(self):
    ...
    self.env.append_value(
        "INTLFLAGS", getattr(self, "flags", self.env.INTLFLAGS_DEFAULT)
    )
...
def apply_intltool_po(self):
    ...
    appname = getattr(
        self, "appname", getattr(Context.g_module, Context.APPNAME, "set_your_app_name")
    )
    podir = getattr(self, "podir", ".")
        ...
        for lang in langs:
            ...
            if re_linguas.match(lang):
                node = self.path.find_resource(
                    os.path.join(podir, re_linguas.match(lang).group() + ".po")
                )
