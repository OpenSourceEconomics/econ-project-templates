...
class tex(Task.Task):
    ...
    makeglossaries_fun, _ = Task.compile_fun(
        "${MAKEGLOSSARIES} ${SRCFILE}", shell=False
    )
    ...
    def bibfile(self):
        ...
        for aux_node in self.aux_nodes:
            ...
            if g_bibtex_re.findall(ct):
                ...
                self.env.env.update(
                    {"BIBINPUTS": self.texinputs(), "BSTINPUTS": self.texinputs()}
                )
    ...
    def makeindex(self):
        ...
        try:
            ...
            self.env.env = {}
            self.check_status(
                "error when calling makeindex %s" % idx_path, self.makeindex_fun()
            )
...
class pdflatex(tex):
    ...
    texfun, vars = Task.compile_fun(
        "${PDFLATEX} ${PDFLATEXFLAGS} ${SRCFILE}", shell=False
    )
    ...
    if getattr(self, "deps", None):
        ...
        for dep in deps:
            if isinstance(dep, str):
                ...
                if not n:
                    self.bld.fatal(f"Could not find {dep!r} for {self!r}")
...
def configure(self):
    ...
    for (
        p
    ) in "tex latex pdflatex xelatex bibtex dvips dvipdf ps2pdf makeindex pdf2ps makeglossaries".split():
        try:
