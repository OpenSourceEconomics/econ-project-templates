...
re_clexer = re.compile(
    "|".join([f"(?P<{name}>{part})" for name, part in zip(tok_types, exp_types)]), re.M
)
...
def paste_tokens(t1, t2):
    ...
    if t1[0] == OP and t2[0] == OP:
        ...
        p1 = NUM
    if not p1:
        raise PreprocError(f"tokens do not make a valid paste {t1!r} and {t2!r}")
    return (p1, t1[1] + t2[1])
            ...
            if isinstance(macro_def[0], list):
                ...
                while j < len(to_add):
                    ...
                    if p2 == OP and v2 == "#":
                        ...
                        if (
                            j + 1 < len(to_add)
                            and to_add[j + 1][0] == IDENT
                            and to_add[j + 1][1] in arg_table
                        ):
    ...
    for y in lst:
        if y:
            ...
            if pos == -1:
                ...
                ret.append("{} {}".format(y[:pos], y[pos + 1 :]))
            else:
    ...
    def start(self, node, env):
        ...
        while self.lines:
            ...
            try:
                ...
                if Logs.verbose:
                    Logs.debug(
                        "preproc: line parsing failed (%s): %s %s",
                        e,
                        line,
