...
def check_same_targets(self):
    ...
    for (k, v) in mp.items():
        if len(v) > 1:
            dupe = True
            msg = "* Node {!r} is created more than once{}. The task generators are:".format(
                k, Logs.verbose == 1 and " (full message on 'waf -v -v')" or ""
            )
            Logs.error(msg)
            for x in v:
                if Logs.verbose > 1:
...
def check_invalid_constraints(self):
    ...
    for cls in list(Task.classes.values()):
        if (
            ...
            and issubclass(cls, Task.Task)
            and isinstance(cls.hcode, str)
        ):
            raise Errors.WafError(
                f"Class {cls!r} has hcode value {cls.hcode!r} of type <str>, expecting <bytes> (use Utils.h_cmd() ?)"
            )
        ...
        if getattr(cls, "rule", None):
            Logs.error(
                'Erroneous attribute "rule" on task class %r (rename to "run_str")',
                cls.__name__,
            )
