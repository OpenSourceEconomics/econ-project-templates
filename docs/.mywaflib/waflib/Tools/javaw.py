...
def jar_files(self):
    ...
    if not basedir:
        self.bld.fatal(f"Could not find the basedir {self.basedir!r} for {self!r}")
    ...
    if manifest:
        ...
        if not isinstance(manifest, Node.Node):
            ...
            node = manifest
        if not node:
            self.bld.fatal(f"invalid manifest file {manifest!r} for {self!r}")
    ...
    if not destfile:
        self.bld.fatal(f"invalid destfile {destfile!r} for {self!r}")
    ...
    def runnable_status(self):
        ...
        if not self.inputs:
            try:
                self.inputs = [
                    x
                    for x in self.basedir.ant_glob(JAR_RE, remove=False)
                    if id(x) != id(self.outputs[0])
                ]
            except Exception:
                raise Errors.WafError(
                    f"Could not find the basedir {self.basedir!r} for {self!r}"
                )
...
class javadoc(Task.Task):
    ...
    def run(self):
        ...
        srcpath += os.pathsep
        srcpath += (
            self.generator.path.get_bld().abspath() + os.sep + self.generator.srcdir
        )
...
def check_java_class(self, classname, with_classpath=None):
    ...
    # Compile the source
    self.exec_command(
        self.env.JAVAC + [os.path.join(javatestdir, "Test.java")], shell=False
    )
...
def check_jni_headers(conf):
    ...
    if dir is None:
        dir = conf.root.find_dir(
            conf.env.JAVA_HOME[0] + "/../Headers"
        )  # think different?!
    if dir is None:
