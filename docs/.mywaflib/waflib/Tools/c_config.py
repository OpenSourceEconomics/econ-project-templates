	...
	* else, execute the *-config* program with the *args* and *variables* given, and set the flags on the *conf.env.FLAGS_name* variable
    ...
    def define_it():
        ...
        if kw.get("global_define", 1):
            ...
            self.env.append_unique(
                "DEFINES_%s" % kw["uselib_store"], "%s=1" % define_name
            )
    ...
    if "modversion" in kw:
        version = self.cmd_and_log(
            path + ["--modversion", kw["modversion"]], env=env
        ).strip()
        if not "okmsg" in kw:
            kw["okmsg"] = version
    ...
    if "variables" in kw:
        ...
        for v in vars:
            ...
            var = "{}_{}".format(kw["uselib_store"], v)
            v_env[var] = val
        return
...
def build_fun(bld):
    ...
    o = bld(
        ...
        source=bld.kw["compile_filename"],
        target="testprog",
    )
...
def validate_c(self, kw):
    ...
    if not "compile_mode" in kw:
        kw["compile_mode"] = "c"
        if (
            "cxx" in Utils.to_list(kw.get("features", []))
    ...
    for (flagsname, flagstype) in (
        ...
        if flagsname in kw:
            if not "msg" in kw:
                kw["msg"] = "Checking for {} flags {}".format(flagstype, kw[flagsname])
            if not "errmsg" in kw:
                kw["errmsg"] = "no"
    ...
    if kw.get("merge_config_header") or env.merge_config_header:
        kw["code"] = "{}\n\n{}".format(self.get_config_header(), kw["code"])
...
def post_check(self, *k, **kw):
    ...
    if kw.get("define_name"):
        ...
        if kw["execute"] and kw.get("define_ret") and isinstance(is_success, str):
            if kw.get("global_define", 1):
                self.define(
                    define_name, is_success, quote=kw.get("quote", 1), comment=comment
                )
            else:
                if kw.get("quote", 1):
                    succ = '"%s"' % is_success
            ...
            if kw.get("global_define", 1):
                ...
                var = "DEFINES_%s" % kw["uselib_store"]
                self.env.append_value(var, "{}={}".format(define_name, int(is_success)))
    ...
    def run(self):
        if getattr(self.generator, "rpath", None):
            if getattr(self.generator, "define_ret", False):
                ...
                self.generator.bld.retval = self.generator.bld.exec_command(
                    [self.inputs[0].abspath()]
                )
        else:
            env = self.env.env or {}
    ...
    if defines:
        ...
        for k in self.env[DEFKEYS]:
            ...
            if caption:
                caption = " /* %s */" % caption
            try:
                txt = "#define {}{} {}{}".format(define_prefix, k, tbl[k], caption)
            except KeyError:
...
def get_suncc_version(conf, cc):
    ...
    try:
        ...
        if not (
            hasattr(e, "returncode") and hasattr(e, "stdout") and hasattr(e, "stderr")
        ):
...
def multicheck(self, *k, **kw):
    ...
    if failure_count:
        self.end_msg(
            kw.get("errmsg", "%s test failed" % failure_count), color="YELLOW", **kw
        )
    else:
