...
class link_task(Task.Task):
    ...
    def exec_mf(self):
        ...
        lst.append("-outputresource:{};{}".format(self.outputs[0].abspath(), mode))

        return super().exec_command(lst)
	...
	* ``libfoo.so.14 → libfoo.so.14.15.16``
    ...
    if not re_vnum.match(self.vnum):
        raise Errors.WafError(
            "Invalid vnum {!r} for target {!r}".format(
