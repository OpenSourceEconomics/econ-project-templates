...
def process_enums(self):
    ...
    for enum in getattr(self, "enums_list", []):
        ...
        for param, option in params.items():
            if enum[param]:
                options.append("{} {!r}".format(option, enum[param]))
...
def add_settings_enums(self, namespace, filename_list):
    ...
    if hasattr(self, "settings_enum_namespace"):
        raise Errors.WafError(
            "Tried to add gsettings enums to %r more than once" % self.name
        )
...
def process_settings(self):
    ...
    for schema in settings_schema_files:
        ...
        schema_task.env.GLIB_COMPILE_SCHEMAS_OPTIONS = [
            ("--schema-file=" + k.abspath()) for k in source_list
        ]
...
class glib_gresource_base(Task.Task):
    ...
    def scan(self):
        ...
        cmd = Utils.subst_vars(
            "${{GLIB_COMPILE_RESOURCES}} --sourcedir={} --sourcedir={} --generate-dependencies {}".format(
...
class glib_gresource_source(glib_gresource_base):
    ...
    fun_h = Task.compile_fun_shell(
        glib_gresource_base.base_cmd
        + " --target=${TGT[0].abspath()} --generate-header ${SRC}"
    )
    fun_c = Task.compile_fun_shell(
        glib_gresource_base.base_cmd
        + " --target=${TGT[1].abspath()} --generate-source ${SRC}"
    )
    ext_out = [".h"]
