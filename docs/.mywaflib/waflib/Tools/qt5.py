...
def apply_qt5(self):
    ...
    if getattr(self, "lang", None):
        ...
        for x in self.to_list(self.lang):
            ...
            qmtasks.append(
                self.create_task("ts2qm", x, x.change_ext(".%d.qm" % self.idx))
            )
        ...
        if getattr(self, "update", None) and Options.options.trans_qt5:
            cxxnodes = [a.inputs[0] for a in self.compiled_tasks] + [
                a.inputs[0]
                for a in self.tasks
                if a.inputs and a.inputs[0].name.endswith(".ui")
            ]
            for x in qmtasks:
...
class rcc(Task.Task):
    ...
    def scan(self):
        ...
        with open(self.inputs[0].abspath()) as f:
            parser.parse(f)
...
def find_qt5_binaries(self):
    ...
    self.env.QT_HOST_BINS = qtbin = self.cmd_and_log(
        self.env.QMAKE + ["-query", "QT_HOST_BINS"]
    ).strip()
    ...
    if not qtlibs:
        try:
            ...
            qtdir = self.cmd_and_log(
                env.QMAKE + ["-query", "QT_INSTALL_PREFIX"]
            ).strip()
...
def find_qt5_libraries(self):
    ...
    qtincludes = (
        self.environ.get("QT5_INCLUDES")
        or self.cmd_and_log(env.QMAKE + ["-query", "QT_INSTALL_HEADERS"]).strip()
    )
    ...
    try:
        ...
        for i in self.qt5_vars:
            ...
            if Utils.unversioned_sys_platform() == "darwin":
                ...
                env.append_unique(
                    "INCLUDES_" + uselib,
                    os.path.join(env.QTLIBS, frameworkName, "Headers"),
                )
            else:
                ret = self.find_single_qt5_lib(
                    i, uselib, env.QTLIBS, qtincludes, force_static
                )
                if not force_static and not ret:
                    ret = self.find_single_qt5_lib(
                        i, uselib, env.QTLIBS, qtincludes, True
                    )
                self.msg("Checking for %s" % i, ret, "GREEN" if ret else "YELLOW")
    else:
        path = "{}:{}:{}/pkgconfig:/usr/lib/qt5/lib/pkgconfig:/opt/qt5/lib/pkgconfig:/usr/lib/qt5/lib:/opt/qt5/lib".format(
