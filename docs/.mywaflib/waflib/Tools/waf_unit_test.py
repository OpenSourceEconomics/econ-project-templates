...
def make_interpreted_test(self):
    ...
    self.ut_run, lst = Task.compile_fun(
        self.test_scripts_template, shell=getattr(self, "test_scripts_shell", False)
    )
...
class utest(Task.Task):
    ...
    def post_run(self):
        super().post_run()
        if (
            getattr(Options.options, "clear_failed_tests", False)
            and self.waf_unit_test_results[1]
        ):
    ...
    def exec_command(self, cmd, **kw):
        ...
        self.waf_unit_test_results = tup = (
...
def summary(bld):
    ...
    if lst:
        ...
        Logs.pprint(
            "GREEN" if tfail == 0 else "RED", "  tests that fail %d/%d" % (tfail, total)
        )
        for (f, code, out, err) in lst:
            if code:
                Logs.pprint("RED", "    %s" % f)
...
def set_exit_code(bld):
    ...
    for (f, code, out, err) in lst:
        if code:
            msg = []
            if out:
                msg.append("stdout:{}{}".format(os.linesep, out.decode("utf-8")))
            if err:
                msg.append("stderr:{}{}".format(os.linesep, err.decode("utf-8")))
