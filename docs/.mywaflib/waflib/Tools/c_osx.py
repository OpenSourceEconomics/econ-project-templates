...
def set_macosx_deployment_target(self):
    ...
    if self.env.MACOSX_DEPLOYMENT_TARGET:
        ...
        if Utils.unversioned_sys_platform() == "darwin":
            os.environ["MACOSX_DEPLOYMENT_TARGET"] = ".".join(
...
def create_task_macapp(self):
    ...
    if self.env.MACAPP or getattr(self, "mac_app", False):
        ...
        inst_to = (
            getattr(self, "install_path", "/Applications")
            + "/%s/Contents/MacOS/" % name
        )
        ...
        if getattr(self, "mac_files", None):
            ...
            inst_to = (
                getattr(self, "install_path", "/Applications") + "/%s/Resources" % name
            )
            for node in self.to_nodes(self.mac_files):
                ...
                self.add_install_as(
                    install_to=os.path.join(inst_to, relpath), install_from=node
                )
...
def create_task_macplist(self):
    ...
    if self.env.MACAPP or getattr(self, "mac_app", False):
        ...
        inst_to = (
            getattr(self, "install_path", "/Applications") + "/%s/Contents/" % name
        )
...
def apply_bundle(self):
    ...
    if self.env.MACBUNDLE or getattr(self, "mac_bundle", False):
        self.env.LINKFLAGS_cshlib = (
            self.env.LINKFLAGS_cxxshlib
