...
def feature_py(self):
    ...
    if not ver:
        self.bld.fatal(
            "Installing python files requires PYTHON_VERSION, try conf.check_python_version"
        )
...
def process_py(self, node):
    ...
    if self.install_path:
        if self.install_from:
            ...
            self.add_install_files(
                install_to=self.install_path, install_from=node, relative_trick=True
            )
    ...
    if self.install_path:
        if self.install_from:
            pyd = Utils.subst_vars(
                "{}/{}".format(self.install_path, node.path_from(self.install_from)),
                self.env,
            )
        else:
            pyd = Utils.subst_vars(
                "{}/{}".format(self.install_path, node.path_from(self.path)), self.env
            )
    else:
        pyd = node.abspath()
    ...
    for ext in lst:
        if self.env.PYTAG and not self.env.NOPYCACHE:
            ...
            pyobj = (
                node.parent.get_bld()
                .make_node("__pycache__")
                .make_node(f"{name}.{self.env.PYTAG}.{ext}")
            )
            pyobj.parent.mkdir()
        else:
...
def init_pyext(self):
	...
	*lib* prefix from library names.
    ...
    ) = self.env.macbundle_PATTERN = self.env.pyext_PATTERN
...
def python_cross_compile(self, features="pyembed pyext"):
    ...
    if not (
        ...
        or "PYTHON_PYEXT_LDFLAGS" in self.environ
    ...
    if "pyext" in features:
        flags = self.environ.get(
            "PYTHON_PYEXT_LDFLAGS", self.environ.get("PYTHON_LDFLAGS")
        )
        if flags is None:
    ...
    if "pyembed" in features:
        flags = self.environ.get(
            "PYTHON_PYEMBED_LDFLAGS", self.environ.get("PYTHON_LDFLAGS")
        )
        if flags is None:
...
def check_python_version(conf, minver=None):
    ...
    assert len(lines) == 5, "found {!r} lines, expected 5: {!r}".format(
        len(lines), lines
    )
        ...
        if "PYTHONDIR" in conf.env:
            ...
            if python_LIBDEST is None:
                if conf.env.LIBDIR:
                    ...
                    python_LIBDEST = os.path.join(
                        conf.env.PREFIX, "lib", "python" + pyver
                    )
    ...
    if minver is None:
        ...
        conf.msg(
            f"Checking for python version >= {minver_str}",
            pyver_full,
            color=result and "GREEN" or "YELLOW",
        )

    if not result:
        conf.fatal(f"The python version is too old, expecting {minver!r}")
...
def check_python_module(conf, module_name, condition=""):
    ...
    conf.start_msg(msg)
    try:
        ret = conf.cmd_and_log(
            conf.env.PYTHON + ["-c", PYTHON_MODULE_TEMPLATE % module_name]
        )
    except Errors.WafError:
        conf.end_msg(False)
...
def configure(conf):
    ...
    try:
        v.PYTAG = conf.cmd_and_log(
            conf.env.PYTHON + ["-c", "import imp;print(imp.get_tag())"]
        ).strip()
    except Errors.WafError:
        pass
