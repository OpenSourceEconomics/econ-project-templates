...
def init_vala_task(self):
    ...
    if hasattr(self, "vala_dir"):
        if isinstance(self.vala_dir, str):
            ...
            try:
                ...
                raise self.bld.fatal(
                    "Cannot create the vala dir %r" % valatask.vala_dir_node
                )
        else:
    ...
    self.vala_target_glib = getattr(
        self, "vala_target_glib", getattr(Options.options, "vala_target_glib", None)
    )
    if self.vala_target_glib:
        addflags("--target-glib=%s" % self.vala_target_glib)

    addflags(
        ["--define=%s" % x for x in Utils.to_list(getattr(self, "vala_defines", []))]
    )
    ...
    valatask.header_path = getattr(
        self, "header_path", f"${{INCLUDEDIR}}/{valatask.pkg_name}-{_get_api_version()}"
    )
    ...
    if hasattr(self, "use"):
        ...
        while len(local_packages) > 0:
            ...
            if hasattr(package_obj, "use"):
                ...
                lst.reverse()
                local_packages = [
                    pkg for pkg in lst if pkg not in seen
                ] + local_packages
    ...
    if self.is_lib and valatask.install_binding:
        ...
        if gir_list:
            self.install_gir = self.add_install_files(
                install_to=getattr(self, "gir_path", "${DATAROOTDIR}/gir-1.0"),
...
def find_valac(self, valac_name, min_version):
    ...
    self.msg(
        f"Checking for {valac_name} version >= {min_version!r}",
        valac_version,
        valac_version and valac_version >= min_version,
    )
    if valac and valac_version < min_version:
        self.fatal(
            f"{valac_name} version {valac_version!r} is too old, need >= {min_version!r}"
        )
