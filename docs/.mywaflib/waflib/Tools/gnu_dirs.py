...
def configure(conf):
    ...
    while not complete and iter < len(_options) + 1:
        ...
        for name, help, default in _options:
            ...
            if not env[name]:
                try:
                    env[name] = Utils.subst_vars(
                        get_param(name, default).replace("/", os.sep), env
                    )
                except TypeError:
                    complete = False
...
def options(opt):
 ...
 than "/usr/local" can be given using "--prefix", for example "--prefix=$HOME"',
    ...
    for name, help, default in _options:
        option_name = "--" + name
        str_default = default
        str_help = "{} [{}]".format(help, re.sub(r"\$\{([^}]+)\}", r"\1", str_default))
        dirs_options.add_option(
            option_name, help=str_help, default="", dest=name.upper()
        )
