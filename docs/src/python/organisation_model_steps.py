...
def path_to(filename):
    return project_paths_join("BLD_PYTHON_EXAMPLE", filename)
...
step_shapes = {"None": OrderedDict(), "full": OrderedDict(), "select": OrderedDict()}
model_shapes = {"None": OrderedDict(), "full": OrderedDict()}
...
for i, item in enumerate(MODEL_CATEGORIES):
    ...
    for graph_type in list(model_shapes.keys()):
        ...
        elif graph_type == "full" or (
            item.startswith("Baseline") or item.startswith("Robust")
        ):
...
def create_fig(out_file, x_items, y_items):
    ...
    ax.set_ylabel(Y_LABEL_TEXT, rotation="horizontal", ha="left", position=(-0.3, 1.05))
    ...
    ax.set_yticklabels(
        list(y_items.keys()), horizontalalignment="left", position=(-0.25, 0)
    )
